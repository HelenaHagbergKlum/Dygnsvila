...
def calculate_weekday_overlap(start, end):
    ...
    if start.time() < datetime.strptime("07:00", "%H:%M").time():
        start += timedelta(days=1)
    if end.time() < datetime.strptime("07:00", "%H:%M").time():
        end += timedelta(days=1)
...
if results:
    ...
    df.loc[len(df.index)] = ["Justerat", "", adjusted_minutes, format_minutes(adjusted_minutes)]
    excel_file = "kompenserad_vila.xlsx"
