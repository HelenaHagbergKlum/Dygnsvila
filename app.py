...
def map_to_dygn(time_str: str) -> datetime:
    """Mappa klockslag till korrekt dygn 07:00–07:00.
    - Tider >= 07:00 hör till dag 0
    - Tider < 07:00 hör till dag 1 (nästa kalenderdag)
    """
    t = parse_time(time_str)
    seven = parse_time("07:00").time()
    if t.time() < seven:
        t += timedelta(days=1)
    return t
...
def calculate_weekday_compensation(start: datetime, end: datetime) -> float:
    """All störningstid inom 20:00–07:00 i samma dygn (07:00–07:00)."""
    start, end = normalize_interval(start, end)
    night_start = parse_time("20:00")                  # D0 20:00
    night_end   = parse_time("07:00") + timedelta(days=1)  # D1 07:00
...
def calculate_weekend_rest(intervals: list) -> float:
    """Brist mot 11h vila inom dygn 07:00–07:00."""
    # intervallen förväntas redan vara mappade till dygn (D0/D1)
...
for day in range(num_days):
    ...
    for i in range(num_intervals):
        ...
        if start_str and end_str:
            try:
                start = map_to_dygn(start_str)
                end = map_to_dygn(end_str)
                start, end = normalize_interval(start, end)
    ...
    if intervals:
        if day_type == "Vardag":
            comp_minutes = sum(calculate_weekday_compensation(s, e) for s, e in intervals)
        else:
