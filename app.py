...
for day in range(num_days):
    ...
    if intervals:
        ...
        total_storning = sum((end - start if end > start else end + timedelta(days=1) - start).total_seconds() / 60 for start, end in intervals)
...
if results:
    ...
    st.download_button("Ladda ner resultat som Excel", data=df.to_excel(index=False, engine='openpyxl'), file_name="kompensation.xlsx")
